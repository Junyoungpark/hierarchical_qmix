...
class BrainBase(nn.Module):
    ...
    @staticmethod
    def clip_and_optimize(optimizer, params, loss, clip_val=None, scheduler=None):
        optimizer.zero_grad()
        loss.backward()
    ...
    def set_optimizer(self, target_opt, lr, params):
        opt = self._get_optimizer(target_opt)
        if target_opt == 'lookahead':
            base_opt = RAdam(params=params, lr=lr)
            optimizer = opt(base_opt)
        else:
            optimizer = opt(params=params, lr=lr)
